<?php
    ...
    function GetAllHaz()
    {
        $query = "SELECT property_description.property_id, property_description.property_sub_type, property_description.property_condition, property_description.property_view, property_description.heating_type, property_description.parking, 
                                    property_description.comfort_rate, property_description.attic_loft, property_description.bathroom_toilet, property_description.balcony_patio, property_description.safe_for_wheelchair, 
                                    property_description.airconditioning, property_description.property_price, property_description.property_size,  property_description.num_of_whole_rooms, property_description.num_of_half_rooms, property_description.num_of_bathrooms,
                                    property_description.plot_size, property_description.num_of_floors, property_description.basement, property_description.attic, 
                                    property.date_on_market, property.status, property.property_type_id 
                                    FROM property_description INNER JOIN property ON property_description.property_id = property.property_id
                                    WHERE property_description.property_type = 'HAZ';";
        $result = select($query, false, null);
        $hazak = array();
        $i = 0;
        foreach ($result as $r) {
        $haz = new Haz( intval($r[0]), $r[1], $r[2], $r[3], $r[4], $r[5], $r[6], $r[7], $r[8], $r[9], $r[10], $r[11], intval($r[12]), intval($r[13]), intval($r[14]), intval($r[15]), intval($r[16]), intval($r[17]), intval($r[18]), $r[19], $r[20]);
            $haz->dateOnMarket = $r[21];
            $haz->status = $r[22];
            $haz->tipus = $r[23];
            $hazak[$i] = $haz;
            $i++;
